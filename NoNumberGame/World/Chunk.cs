...
namespace NoNumberGame
{
	public class Chunk
	{
		public const int Chunk_Width = 256;
		public const int Chunk_Width_One = 255;
		...
		private Chunk( int x, int z, float[] heightmap ) {
			X = x;
			Z = z;
		...
		public Mesh GenerateMesh() {
			...
			for ( int z = 0; z < 256; ++z )
			for ( int x = 0; x < 256; ++x ) {
				float y = _heightmap[x + 256 * z];

				vertexArray[3 * ( x + 256 * z ) + 0] = Chunk_Width_One * X + x - 0.0f;
				vertexArray[3 * ( x + 256 * z ) + 1] = y * 64.0f;
				vertexArray[3 * ( x + 256 * z ) + 2] = Chunk_Width_One * Z + z - 0.0f;
			...
			return new Mesh( vertexArray, normalArray, colorArray, texcoordArray, indexArray );
		}

		public static Chunk Generate( int chunkX, int chunkZ ) {
			return new Chunk( chunkX, chunkZ, NoiseGenerator.GenerateNoise2D( Chunk_Width * chunkX, Chunk_Width * chunkZ, Chunk_Width, Chunk_Width,
				4, new uint[] { 2, 8, 32, 128 }, new[] { 0.04f, 0.1f, 1.0f, 0.4f } ) ); //TODO layers, octaves, amplitudes -> generator settings
		}
	}
}