...
namespace NoNumberGame
{
	public readonly struct Mesh
	{
		...
		private readonly float[] _texcoordArray;
		...
		public Mesh( int vertices, int indices ) {
			...
			_texcoordArray = new float[2 * vertices];
			_indexArray    = new int[indices];
		}

		public Mesh( float[] vertexArray, float[] normalArray, float[] colorArray, float[] texcoordArray, int[] indexArray ) {
		...
		public Vao GenerateVao() {
			...
			GL.BufferData( BufferTarget.ArrayBuffer, _normalArray.Length * sizeof ( float ), _normalArray, BufferUsageHint.StaticCopy );
			...
			GL.BufferData( BufferTarget.ArrayBuffer, _texcoordArray.Length * sizeof ( float ), _texcoordArray, BufferUsageHint.StaticCopy );
